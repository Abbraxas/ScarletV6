...
from ScarletV5.utils.database import get_loop
from ScarletV5.utils.decorators import AdminRightsCheck
from ScarletV5.utils.inline import close_markup, stream_markup, stream_markup2
    ...
    if "live_" in queued:
        ...
        if videoid == "telegram":
            ...
            run = await message.reply_photo(
                ...
                caption=_["stream_1"].format(
                    C_SUPPORT_CHAT, title[:23], check[0]["dur"], user, message.from_user.id
                ),
                reply_markup=InlineKeyboardMarkup(button),
            )
            db[chat_id][0]["mystic"] = run
