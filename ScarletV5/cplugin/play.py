...
import config
from ScarletV5 import Apple, Resso, SoundCloud, Spotify, Telegram, YouTube, app
from ScarletV5.core.call import Axiom
from ScarletV5.misc import SUDOERS
from ScarletV5.utils.inline import panel_markup_clone
from ScarletV5.utils import seconds_to_min, time_to_seconds
from ScarletV5.utils.channelplay import get_channeplayCB
from ScarletV5.utils.decorators.language import languageCB
from ScarletV5.utils.decorators.play import CPlayWrapper
from ScarletV5.utils.formatters import formats
from ScarletV5.utils.inline import (
    botplaylist_markup,
    livestream_markup,
    playlist_markup,
    slider_markup,
...
from ScarletV5.utils.database import (
...
import config
from ScarletV5 import Carbon, YouTube
from ScarletV5.core.call import Axiom
from ScarletV5.misc import db
from ScarletV5.utils.database import add_active_video_chat, is_active_chat
from ScarletV5.utils.exceptions import AssistantErr
from ScarletV5.utils.inline import (
    aq_markup,
    queuemarkup,
    close_markup,
...
from ScarletV5.utils.pastebin import AxiomBin
from ScarletV5.utils.stream.queue import put_queue, put_queue_index
...
async def stream(
    ...
    if not result:
        return
    if forceplay:
        await Axiom.force_stop_stream(chat_id)
    if streamtype == "playlist":
        ...
        if await is_active_chat(chat_id):
            ...
            await client.send_message(
                ...
                text=_["queue_4"].format(position, title[:18], duration_min, user_name, f"https://t.me/{a.username}?start=info_{vidid}", user_id),
                reply_markup=InlineKeyboardMarkup(button),
        ...
        if await is_active_chat(chat_id):
            ...
            await client.send_message(
                chat_id=original_chat_id,
                text=_["queue_4"].format(position, title[:18], duration_min, user_name, C_SUPPORT_CHAT, user_id),
                reply_markup=InlineKeyboardMarkup(button),
                disable_web_page_preview=True,
            )
        else:
            if not forceplay:
                db[chat_id] = []
            await Axiom.join_call(chat_id, original_chat_id, file_path, video=None)
            await put_queue(
                chat_id,
                original_chat_id,
                file_path,
        ...
        if await is_active_chat(chat_id):
            ...
            await client.send_message(
                chat_id=original_chat_id,
                text=_["queue_4"].format(position, title[:18], duration_min, user_name, f"https://t.me/{a.username}?start=info_{streamtype}", user_id),
                reply_markup=InlineKeyboardMarkup(button),
                disable_web_page_preview=True,
            )
        else:
            if not forceplay:
                db[chat_id] = []
            await Axiom.join_call(chat_id, original_chat_id, file_path, video=status)
            await put_queue(
                chat_id,
                original_chat_id,
                file_path,
